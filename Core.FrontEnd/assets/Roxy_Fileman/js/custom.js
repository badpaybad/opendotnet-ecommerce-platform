...
function FileSelected(file) {
     ...
     * 
     */
    var senderDomId = RoxyUtils.GetUrlParam('senderDomId');

    if (senderDomId != null && senderDomId != 'undefined') {
        $(window.parent.document).find('#' + senderDomId).attr('src', file.fullPath);
    }


    var receiverDomId = RoxyUtils.GetUrlParam('receiverDomId');
    if (receiverDomId != null && receiverDomId != 'undefined') {
        $(window.parent.document).find('#' + receiverDomId).attr('value', file.fullPath);
    }

    window.parent.RoxyFileMan.closeCustomRoxy();
}
