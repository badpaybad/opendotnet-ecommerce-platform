...
{
    [AdminLoginRequired()]
    public class AdminRoxyFilemanController : AdminBaseController
    {
        #region Fields
        ...
        string confFile = "~/assets/Roxy_Fileman/conf.json";
        ...
        //custom code by nopCommerce team
        public AdminRoxyFilemanController()
        {
        ...
        protected string GetFilesRoot()
        {
            ...
            {
                if (_context.Session[GetSetting("SESSION_PATH_KEY")] != null)
        ...
        public void DownloadDir(string path)
        {
            ...
            System.IO.Compression.ZipFile.CreateFromDirectory(path, tmpZip, CompressionLevel.Fastest, true);
            _r.Clear();
        ...
        {
            CheckPath(path);
            string serverPath = FixPath(path);
            ...
            try
            {
                for (int i = 0; i < Request.Files.Count; i++)
                {
                    if (CanHandleFile(Request.Files[i].FileName))
                    {
                        ...
                        string filename = MakeUniqueFilename(serverPath, f.Name);
                        string dest = Path.Combine(serverPath, filename);
                        Request.Files[i].SaveAs(dest);

                        //todo: Du comment, discuss late  insert image
